...
module SeedsHelperTaskDB
  ...
  def self.execute
    ...
    init_task_status.each { |info|
      ...
      info[:task_status][:next_task_status_id] = TaskStatus.find_by(:name_jp => task_flow_info[:next_task_status]).id if task_flow_info[:next_task_status] != nil
      info[:task_status][:prev_task_status_id] = TaskStatus.find_by(:name_jp => task_flow_info[:prev_task_status]).id if task_flow_info[:prev_task_status] != nil
